...
df = pd.DataFrame(act, columns=['activityId', 'startTimeLocal', 'distance', 'averageHR', 'total time', 'zones 1 and 2 %'])
...
fig = df.groupby('date').sum()['rosinski-hubar index'].plot(figsize = (10,10), title='running').get_figure()
